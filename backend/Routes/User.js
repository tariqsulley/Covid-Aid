...
// Analysing against the database

router.route("/analyse").post((req, res) => {
  let currentUser;
  let currentIdentifier = req.body.identifier;
  let hasCovid = false;
  Users.findOne({ uniqueIdentifier: currentIdentifier })
    .then((data) => {
      currentUser = data;
    })
    .catch((err) => {
  ...
  Users.find({ uniqueIdentifier: { $ne: currentIdentifier } })
    .then((data) => {
      data.forEach((person) => {
        diffLong = [];
        diffLat = [];
...
router.route("/:id").put((req, res) => {
  Users.find({ uniqueIdentifier: req.params.id }, function (err, user) {
    if (err) res.send(err);
    req.body.longitude ? (user.longitude = req.body.longitude) : "";
    req.body.latitude ? (user.latitude = req.body.latitude) : "";
